...
(()=>{
  ...
  function hideApiKey(){
    ...
    if(keyField){const wrap=keyField.closest('div');if(wrap)wrap.style.display='none';}
  ...
  function updateStatus(){
    ...
    if(st){
      st.className='integrationStatus '+(connected?'ok':'warn');
      st.textContent=connected?'Drive conectado':hasClient?'OAuth v151 configurado':'OAuth v151 não configurado';
    }
  ...
  function clearConfig(){
    ...
    setLegacyToken('');
    updateStatus();
    status('Configuração Google Drive removida deste navegador.');
    toast('Configuração Google Drive removida','ok',3000);
  }
  ...
  async function connectDrive(){
    ...
    try{
      ...
      try{if(typeof gdriveUpdateUI==='function')gdriveUpdateUI(true);}catch(e){}
      updateStatus();
      toast('Google Drive conectado ✅','ok',3500);
      try{ if(typeof audit==='function') audit('Google Drive conectado via OAuth v151.'); }catch(e){}
      return token;
    }catch(e){
      setLegacyToken('');
      try{if(typeof gdriveUpdateUI==='function')gdriveUpdateUI(false);}catch(_e){}
      updateStatus();
  ...
  function disconnectDrive(){
    ...
    try{if(typeof gdriveUpdateUI==='function')gdriveUpdateUI(false);}catch(e){}
    updateStatus();
    const info=document.getElementById('driveUserInfo');if(info)info.style.display='none';
  ...
  async function testConfig(){
    ...
    const id=clientId();
    if(!id || !id.endsWith('.apps.googleusercontent.com')){
      toast('Informe um OAuth Client ID válido','err',5000);return;
    }
  ...
  // Compatibilidade: o motor nativo continua usando estas funções/globais.
  try{window.gdriveClientId=clientId;}catch(e){}
  try{window.gdriveApiKey=()=>'';}catch(e){}
  window.loadIntegrationConfigs=loadConfigs;
  window.updateIntegrationStatus=updateStatus;
  window.saveGDriveIntegrationConfig=saveConfig;
  window.clearGDriveIntegrationConfig=clearConfig;
  ...
  window.addEventListener('pp:gdrive-connected',()=>{
    setLegacyToken(window.PortariaGDriveOAuth?.token?.()||'');
    try{if(typeof gdriveUpdateUI==='function')gdriveUpdateUI(true);}catch(e){}
    updateStatus();
  });
  ...
  if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',loadConfigs,{once:true});
  else setTimeout(loadConfigs,0);

  window.PortariaGDriveAdapter={
    version:'151.12-consolidado',
    key:KEY,
